...
impl DeviceService {
    pub fn new() -> Self {
        DeviceService { registry: Mutex::new(None) }
    }

    pub fn with_registry(registry: DeviceRegistry) -> Self {
        DeviceService { registry: Mutex::new(Some(registry)) }
    }
    ...
    fn use_registry<T>(&self, f: impl FnOnce(&DeviceRegistry) -> T) -> Result<T, IpcError> {
        ...
        match guard.as_ref() {
            ...
            None => Err(IpcError::new("registry_unavailable", "device registry not initialized")),
        }
    }
    ...
    pub fn enroll(&self, token: &str, user_agent: &str, mobile: bool, max_devices: usize) -> Result<String, IpcError> {
        let now = now_ms();
        let consumed = self
.use_registry(|registry| registry.consume_token(token, now))
            .map_err(|e| e)
            .and_then(|res| res.map_err(|e| IpcError::new("token_consume_failed", e.to_string())))?;
        if !consumed {
            return Err(IpcError::new("invalid_or_expired_token", "invalid or expired token"));
        }
        let id = random_hex();
        ...
        self.use_registry(|registry| registry.register_device(&id, &name, platform, &auth_secret, now))
            .map_err(|e| e)
    ...
    pub fn auth_secret_for(&self, id: &str) -> Result<Option<String>, IpcError> {
        self.use_registry(|registry| registry.get_device(id))
            ...
            .map(|record| record.filter(|d| d.revoked_at.is_none()).map(|d| d.auth_secret.clone()))
    }
    ...
    pub fn is_online(&self, id: &str, offline_after_ms: i64) -> Result<bool, IpcError> {
        ...
        self.use_registry(|registry| registry.get_device(id))
            ...
            .map(|device| match device {
                Some(device) => device.revoked_at.is_none() && now - device.last_seen <= offline_after_ms,
                None => false,
            })
    }
}
...
pub fn dispatch(method: &str, params: &crate::mini_json::JsonValue, service: &DeviceService) -> Result<Option<String>, IpcError> {
    ...
    match method {
        "device.authSecret" => {
            ...
            service.auth_secret_for(&id).map(|secret| match secret {
                Some(secret) => Some(format!("{{\"secret\":{}}}", crate::ipc_server::json_str(&secret))),
                None => Some("{\"secret\":null}".to_string()),
            })
        }
        "device.issue" => {
            let ttl = params.get(&["ttlMs"]).and_then(|v| v.as_num()).map(|n| n as i64).unwrap_or(600_000);
            service.issue_token(ttl).map(|token| Some(format!("{{\"token\":{},\"expiresAt\":{}}}", json_str(&token), now_ms() + ttl)))
        }
        "device.enroll" => {
            let token = str_param(params, "token");
            let user_agent = str_param(params, "userAgent");
            let mobile = params.get(&["mobile"]).and_then(|v| v.as_bool()).unwrap_or(false);
            let max_devices = params.get(&["maxDevices"]).and_then(|v| v.as_num()).map(|n| n as usize).unwrap_or(4);
            service.enroll(&token, &user_agent, mobile, max_devices).map(|id| Some(format!("{{\"id\":{}}}", json_str(&id))))
        }
        "device.touch" => {
            let id = str_param(params, "id");
            service.touch(&id).map(|ok| Some(format!("{{\"ok\":{}}}", ok)))
        }
        "device.revoke" => {
            let id = str_param(params, "id");
            service.revoke(&id).map(|ok| Some(format!("{{\"ok\":{}}}", ok)))
        }
        "device.revokeAll" => {
            service.revoke_all().map(|()| Some("{\"ok\":true}".to_string()))
        }
        "device.list" => {
            let offline_after = params.get(&["offlineAfterMs"]).and_then(|v| v.as_num()).map(|n| n as i64).unwrap_or(25_000);
...
mod tests {
    ...
    fn issue_enroll_touch_revoke_lifecycle() {
        ...
        let id = service.enroll(&token, "Mozilla/5.0 (iPhone)", true, 4).unwrap();
        assert_eq!(id.len(), 32);
    ...
    fn list_marks_offline_and_revoke_all_clears() {
        ...
        let id = service.enroll(&token, "Mozilla/5.0 (Windows NT 10.0)", false, 4).unwrap();
    ...
    fn max_devices_evicts_oldest() {
        ...
        for i in 0..3 {
            let token = service.issue_token(60_000).unwrap();
            let id = service.enroll(&token, &format!("UA {i}"), false, 2).unwrap();
            let _ = id;
        }
        let listed = service.list(25_000).unwrap();
