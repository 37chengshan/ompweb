...
fn main() {
    ...
    match mode {
        ...
        "--ipc" | "ipc" => {
            ...
            let supervisor = std::sync::Arc::new(supervisor::Supervisor::new(resolve_omp_bin(), vec![]));
            ...
            let device_registry = ompweb_storage::DeviceRegistry::open(std::path::Path::new(&journal_path))
                .expect("device registry open");
            let device_service = std::sync::Arc::new(device_service::DeviceService::with_registry(device_registry));
            ...
            let remote_executor: remote_runtime::Executor = std::sync::Arc::new(move |_device, command| {
                let command_value = mini_json::JsonValue::parse(command).unwrap_or(mini_json::JsonValue::Null);
                let session_id = command_value.get(&["sessionId"]).and_then(|v| v.as_str()).unwrap_or("").to_string();
                if session_id.is_empty() {
                    return Err("agent command requires sessionId".into());
                }
                supervisor_handle.send(&session_id, command).map(|()| "null".to_string()).map_err(|e| e.to_string())
            });
            ...
            let remote_bind = std::env::var("OMP_WEB_REMOTE_BIND").unwrap_or_else(|_| "127.0.0.1:0".to_string());
            let remote_listener = std::net::TcpListener::bind(&remote_bind)
                .expect("remote ws bind");
            ...
            remote_listener.set_nonblocking(true).expect("remote nonblocking");
            std::thread::spawn(move || {
                let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
                rt.block_on(async move {
                    let listener = tokio::net::TcpListener::from_std(remote_listener).expect("tokio listener");
            ...
            let handler: std::sync::Arc<ipc_server::Handler> = std::sync::Arc::new(move |method, params, emit| {
                match method {
                    ...
                    "device.issue" | "device.enroll" | "device.touch" | "device.revoke" | "device.revokeAll" | "device.list" | "device.authSecret" => {
                        device_service::dispatch(method, params, &device_service).map(|body| body.map(Into::into))
                    }
                    "commands.run" => {
                        ...
                        let detach = params.get(&["detach"]).and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        "--files-list" | "files-list" => {
            ...
            match file_service::cli_list(dir) {
                Ok(body) => { let _ = writeln!(std::io::stdout(), "{body}"); }
                Err(err) => {
