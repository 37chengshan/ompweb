...
test("autolinked URLs do not swallow trailing CJK text or punctuation", () => {
  const tree = {
    type: "root",
    children: [{ type: "paragraph", children: [{ type: "link", url: "https://github.com/lizhipay/acg-faka，这个项目如何", children: [{ type: "text", value: "https://github.com/lizhipay/acg-faka，这个项目如何" }] }] }],
  };
  remarkPathLinks()(tree);
  const children = tree.children[0].children;
  assert.equal(children[0].type, "link");
  assert.equal(children[0].url, "https://github.com/lizhipay/acg-faka");
  assert.equal(children[0].children[0].value, "https://github.com/lizhipay/acg-faka");
  // The swallowed text is restored as a sibling text node.
  assert.equal(children[1].type, "text");
  assert.equal(children[1].value, "，这个项目如何");
});
