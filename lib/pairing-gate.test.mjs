...
test("pairing gate: forged Host: localhost does not classify a remote socket as loopback", () => {
  // Attacker connects from 192.168.1.50 but sends Host: localhost.
  const req = fakeRequest({ host: "localhost", "x-forwarded-for": "192.168.1.50" });
  assert.equal(isLoopbackConnection(req), false, "remote socket must stay remote");
});

test("pairing gate: real loopback socket is loopback regardless of Host header", () => {
  const req = fakeRequest({ host: "192.168.1.50", "x-forwarded-for": "127.0.0.1" }, "127.0.0.1");
...
test("pairing gate: trusted-proxy headers only honored when explicitly enabled", () => {
  ...
  try {
    // Without the flag, cf-connecting-ip is NOT trusted → falls back to the
    // packet source (unknown here) → conservative loopback.
    delete process.env.OMP_WEB_TRUSTED_PROXY;
    const req = fakeRequest({ host: "localhost", "cf-connecting-ip": "192.168.1.5" });
    assert.equal(isLoopbackConnection(req), true, "untrusted CF header must be ignored");
...
test("pairing gate: ::1 and 127.x are loopback", () => {
