...
test("Linux slash-less relative files open the current directory, never the root", async () => {
  const { buildRevealCommand } = await loadSubject();
  assert.equal(buildRevealCommand("linux", "README.md", false), "xdg-open '.'");
  assert.equal(buildRevealCommand("linux", "/etc/passwd", false), "xdg-open '/etc'");
  assert.equal(buildRevealCommand("linux", "/", false), "xdg-open '/'");
});